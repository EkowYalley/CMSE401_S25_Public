...
char ** process_img(char ** img, char ** output, image_size_t sz, int halfwindow, double thresh) {
    ...
    #pragma omp parallel for schedule(auto)       // Static scheduling

    // schedule(static, 16)   // Static scheduling with chunk size of 16

    // schedule(dynamic)      // Dynamic scheduling

    // schedule(dynamic, 8)   // Dynamic scheduling with chunk size of 8

    // schedule(guided)       // Guided scheduling

    // schedule(guided, 4)    // Guided scheduling with minimum chunk size of 4

    // schedule(runtime)      // Schedule from OMP_SCHEDULE environment variable
    ...
    #pragma omp parallel for schedule(auto) // Add scheduling option here
